...
SERVER_IP = "10.21.74.238"
SERVER_PORT = 8889
DES_KEY = ""
encryption_key_hex = ""
decryption_key_hex = ""
initiator_p = 81603467655976593886027896926514578073026881618698717465413797227575506058756489360393270235671780572459014373680301703581054910911619631912960595992970967695690246419145276379625317014760834449999951
initiator_q = 31650234949118950681442463325363598512909937683991710377775853328580031089549824137471650036088766117856524233782707541751588679307152398655746701618956200582514102319921683856626763384902734099058493
initiator_e = 65537
initiator_n = initiator_p * initiator_q
...
def start_client():
    ...
    try:
        ...
        # Generate RSA
        initiator_d = generate_rsa(initiator_p, initiator_q, initiator_e)
        
        # Key Distribution
        # Share initiator public key (PUa) to responder
        PUa = {
            'e': initiator_e,
            'n': initiator_n
        }
        logging.info(f"[CLIENT] sharing public key PUa-e:{PUa['e']}, PUa-n:{PUa['n']} to responder..")
        sock.sendall(str(PUa).encode())
        # Receive responder public key (PUb) from responder
        recv_data = sock.recv(4096)
        PUb = eval(recv_data.decode())
        logging.info(f"[CLIENT] received responder public key: PUb-e:{PUb['e']}, PUb-n:{PUb['n']}")

        # Step 1 (ID A = IP, port)
        N1 = str(uuid.uuid4())
        logging.info(f"[CLIENT] N1: {N1}")

        initiator_ip, initiator_port = sock.getsockname()
        msg1 = {
            'sender_ip': initiator_ip,
            'sender_port': initiator_port,
            'message1': N1
        }
        logging.info(f"[CLIENT] sending msg1: {str(msg1)}")
        sock.sendall(repr(encrypt_rsa(int.from_bytes(((str(msg1)).encode('utf-8'))), PUb['e'], PUb['n'])).encode())

        # Step 2 - receive msg2 from responder
        recv_data = sock.recv(4096)
        msg2 = decrypt_rsa(int(recv_data.decode()), initiator_d, initiator_n)
        msg2_bytes = msg2.to_bytes((msg2.bit_length() + 7) // 8)
        msg2 = eval(msg2_bytes.decode('utf-8'))
        logging.info(f'[CLIENT] receives msg2: {str(msg2)}')

        assert N1 == msg2['message1']

        # Step 3
        msg3 = {
            'message3': msg2['message2']
        }
        logging.info(f"[CLIENT] sending msg3: {str(msg3)}")
        sock.sendall(repr(encrypt_rsa(int.from_bytes(((str(msg3)).encode('utf-8'))), PUb['e'], PUb['n'])).encode())

        # Step 4 - receive msg4 from responder
        recv_data = sock.recv(4096)
        msg4 = decrypt_rsa(int(recv_data.decode()), initiator_d, initiator_n)
        msg4_bytes = msg4.to_bytes((msg4.bit_length() + 7) // 8)
        msg4 = eval(msg4_bytes.decode('utf-8'))
        logging.info(f'[CLIENT] receives msg4: {str(msg4)}')

        assert N1 == msg4['message1']
        global DES_KEY, encryption_key_hex, decryption_key_hex
        DES_KEY = msg4['symmetric_key']
        encryption_key_hex = msg4['symmetric_key']
        decryption_key_hex = msg4['symmetric_key']
        assert msg1 and msg2 and msg3 and msg4

        logging.info(f"[CLIENT] key distribution complete!")
